...
export default function ChristmasDecorations() {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[40] overflow-hidden">
      ...
      {/* Snow layer */}
      <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-b from-white/30 to-transparent opacity-60" />
      {snowflakes.map((flake) => (
        <div
          key={flake.i}
          className="absolute top-[-20px] rounded-full bg-white/90 blur-[1px]"
          style={{
            left: flake.left,
            width: flake.size,
            height: flake.size,
      ...
      <div className="absolute top-0 left-0 w-full flex justify-between px-2 overflow-hidden">
        {[...Array(25)].map((_, i) => (
          <div
            key={i}
            className={`w-2 h-2 rounded-full mt-[-4px] shadow-md ${
              i % 2 === 0
      ...
      {/* Santa fly-by */}
      <div
        className="absolute top-24 left-0 text-6xl opacity-40 drop-shadow-lg"
        style={{ animation: 'fly-santa 45s linear infinite' }}
      >
        🦌🎅🛷
      </div>
    </div>
